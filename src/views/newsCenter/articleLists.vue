<template>
  <div class="ad-box">
    ...
    <div class="table-box">
        ...
        <el-button @click="delBtn(2)" plain type="primary">添加文章</el-button>
        <el-table
            size="medium"
...
export default {
  ...
  methods: {
    ...
    getData(){
        ...
        this.ruleForm.according.toString()?dataParams.release = this.ruleForm.according:'';
        this.ruleForm.top.toString()?dataParams.top = this.ruleForm.top:'';
        this.ruleForm.order.toString()?dataParams.recommend = this.ruleForm.order:'';
    ...
    delBtn(i){
        ...
        if(i==1){
            if(this.selectionTab.length>0){
                ...
                this.axios.delete(`news/delete/`,{params:{news_ids:seleteId.join(',')}}).then(res=>{
            ...
            this.$router.push({
                path: `/main/newsCenter/articleLists/addOrEdit/-1`,
            })
        }
    },
    edit(row){
        this.$router.push({
            path: `/main/newsCenter/articleLists/addOrEdit/${row.id}`,
        })
    },
